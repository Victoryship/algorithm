...
示例 2：
	...
	输出：9
	// 转移方程： dp[i] = min(leftMax[i], rigthMax[i]) - height[i]
*/
func Trap(height []int) int {
	...
	dpLeftMax, dpRightMax := make([]int, length), make([]int, length)
	dpLeftMax[0], dpRightMax[length-1] = height[0], height[length-1]
	for i := 1; i < length; i++ {
		dpLeftMax[i] = dpLeftMax[i-1]
		if height[i] > dpLeftMax[i] {
			dpLeftMax[i] = height[i]
		}
	}

	for i := length - 2; i > 0; i-- {
		dpRightMax[i] = dpRightMax[i+1]
		if height[i] > dpRightMax[i] {
			dpRightMax[i] = height[i]
		}
	}

	for i := 1; i < length-1; i++ {
		if dpRightMax[i] < dpLeftMax[i] {
			res += dpRightMax[i] - height[i]
			continue
		}

		res += dpLeftMax[i] - height[i]
	}
